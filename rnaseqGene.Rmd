...
BiocStyle::markdown()
options(width=100)
opts_chunk$set(message = FALSE, error = FALSE) ## matter of taste but wh likes these better
## ML: I want warnings because makeTranscriptDbFromGFF gives 2 benign warnings
## and I don't want users to be surprised.
```
...
We will continue, using *DESeq2*. The *SummarizedExperiment* object is
all we need to start our analysis. In the following section we will
show how to create the data object which is used in *DESeq2*, either
using the *SummarizedExperiment*, or in general, a count matrix which
has been loaded into R.
...
colors <- colorRampPalette( rev(brewer.pal(9, "Blues")) )(255)
heatmap.2( samplePoisDistMatrix, trace="none", col=colors,
          margins=c(2,10), labCol=FALSE )
```
...
methods from the `r Biocpkg("sva")` or `r Biocpkg("RUVSeq")` package to
...
Once we are finished, we detach *RUVSeq* and its dependencies
so that we can continue to use the *plotPCA* and *plotMA* functions
defined in *DESeq2*. 
...
Here we demontrate with the `r Biocexptpkg("fission")` data package,
...
The following chunk performs a likelihood ratio test.
