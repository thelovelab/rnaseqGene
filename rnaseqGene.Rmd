...
*SummarizedExperiment* class.  One of the main differences is that the
...
example of building up such a plot with the *ggplot* function from the
...
## Detecting hidden batches or other unwanted variation
...
methods from the `r Biockg("sva")` or `r Biocpkg("RUVSeq")` package to
...
software uses the term *surrogate variables* and the RUV software uses
the term *factors or unwanted variation*. We first show the use of SVA:
...
design(ddsruv) <- ~ dex
ddsruv <- DESeq(ddsruv)
idx <- rowMeans(counts(ddsruv)) > 1
object <- list(counts = counts(ddsruv)[idx,],
               fitted.values = assays(ddsruv)[["mu"]][idx,],
               dispersion = dispersions(ddsruv)[idx])
object <- new("DGEGLM", object)
resid <- residuals(object)
```

```{r}
set <- newSeqExpressionSet(object$counts)
genes <- rownames(object$counts)
ruv <- RUVr(set, genes, k=2, resid)
```

As before we can compare how well the RUV factors match the known cell 
line information:

```{r}
par(mfrow=c(2,1),mar=c(5,5,1,1))
stripchart(pData(ruv)[,1] ~ dds$cell,vertical=TRUE)
abline(h=0)
stripchart(pData(ruv)[,2] ~ dds$cell,vertical=TRUE)
abline(h=0)
```

```{r}
ddsruv$W1 <- pData(ruv)[,1]
ddsruv$W2 <- pData(ruv)[,2]
design(ddsruv) <- ~ W1 + W2 + dex
ddsruv <- DESeq(ddsruv)
head(results(ddsruv),4)
```
