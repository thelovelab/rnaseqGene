...
a current list. We recommend consulting benchmarking papers that
...
```{r}
csvfile <- paste0(dir,"/sample_table.csv")
...
features, e.g. GFF3 or GTF files specifying a gene model.
...
*featureCounts*     | `r Biocpkg("Rsubread")`                              | R/Bioconductor | count matrix           | *DESeqDataSetFromMatrix*
*htseq-count*       | [HTSeq](http://www-huber.embl.de/users/anders/HTSeq) | Python         | count files            | *DESeqDataSetFromHTSeq* 
...
```{r}
filenames <- paste0(dir,"/",sampleTable$Run, "_subset.bam")
```
...
*makeTranscriptDbFromGFF* from the `r Biocpkg("GenomicFeatures")`
...
The *makeTranscriptDbFromBiomart* function can be used to automatically
...
The `colData` slot, so far empty, should contain all the meta
data. We hence assign our sample table to it:

```{r}
(colData(se) <- DataFrame(sampleTable))
```

This *SummarizedExperiment* object is then all we need to start our
analysis. In the following section we will show how to create the data
object which is used in *DESeq2*, either using the
*SummarizedExperiment*, or in general, a count matrix which has been
loaded into R.

## The DESeqDataSet, column metadata, and the design formula

Bioconductor software packages often have a special class of data
object, which contains special slots and requirements. The data object
class in *DESeq2* is the *DESeqDataSet*, which is built on top of the
*SummarizedExperiment* class.  One main differences is that the
`assay` slot is instead accessed using the *count* accessor, and the
values in this matrix must be non-negative integers.
...
the parathyroid experiment, we will specify `~ cell + dex`, which
...
* more generally, from a count matrix (i.e. matrix) and a column
metadata table which have been loaded into R

For a full example of using the HTSeq Python package for read
counting, please see the `r Biocexptpkg("pasilla")`.  For an example
of generating the *DESeqDataSet* from files produced by *htseq-count*,
please see the `r Biocpkg("DESeq2")` vignette.
...
## Visualing exploring the dataset
...
stabilizing transformation*, is discussed alonside the *rlog* in the
*DESeq2* vignette. 
...
```{r distheatmap, fig.width=7, fig.height=6}
...
heatmap.2( sampleDistMatrix, trace="none", col=colors, margins=c(5,10) )
```
...
The two terms specified as *intgroup* are the interesting groups for
...
From both visualizations, we see that the differences between cells is
...
We can also summarize the results with the following call, which
...
effect, so a more detailed plot would also show the cell lines. An
example of building up such a plot with the *ggplot* function:

```{r ggplotcounts, fig.height=5}
library("ggplot2")
...
cols <- colorRampPalette( rev(brewer.pal(9, "PuOr")) )(255)
sidecols <- c("grey","dodgerblue")[ rld$dex ]
mat <- assay(rld)[ topVarGenes, ]
mat <- mat - rowMeans(mat)
colnames(mat) <- paste0(rld$dex,"-",rld$cell)
heatmap.2(mat, trace="none", col=cols, ColSideColors=sidecols,
...
Finally, we note that you can easily save the results table in a CSV
file, which you can then load with a spreadsheet program such as
Excel. The call to *as.data.frame* is necessary to convert the
*DataFrame* object (`r Biocpkg("IRanges")` package) to a *data.frame*
object which can be processed by *write.csv*. 

```{r eval=FALSE}
write.csv(as.data.frame(res), file="results.csv")
```

## Detecting hidden batches or unwanted variation

```{r}
dat <- counts(dds)[rowMeans(counts(dds)) > 1,]
mod <- model.matrix(~ dex, colData(dds))
mod0 <- model.matrix(~ 1, colData(dds))
svseq <- svaseq(m,mm1,mm0,n.sv=3)
svseq$sv
```
...
ceases to work because a package has been changed in a newer
version. The session information should also **always** be included in any
emails to the
