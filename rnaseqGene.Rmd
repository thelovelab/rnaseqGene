---
author: Michael I. Love, Simon Anders, Wolfgang Huber
date: August 14, 2015
...
`r Biocpkg("EBSeq")`, and `r Biocpkg("BaySeq")`.
...
are used, for example, in asthma patients to reduce
...
counting reads/fragments. We will read the gene model from a
...
ranges, and the `colData` (green block) contains information about the
...
*regularized-logarithm transformation*, or *rlog* for short. For genes
with high counts, the rlog 
...
Another transformation, the *variance stabilizing transformation*, is
discussed alongside the *rlog* in the *DESeq2* vignette. 
...
*Poisson Distance*, implemented in the CRAN package
...
```{r plotma, fig.width=5, fig.height=5}
...
```{r plotmalfc, fig.width=5, fig.height=5}
...
```{r plotmalabel, fig.width=5, fig.height=5}
plotMA(resLFC1, ylim=c(-5,5))
...
```{r plotdispests, fig.width=5, fig.height=5}
plotDispEsts(dds)
```
...
```{r histpvalue, fig.width=5, fig.height=5}
...
```{r histpvalue2, fig.width=5, fig.height=5}
