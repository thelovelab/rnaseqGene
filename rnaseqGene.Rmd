...
Bioconductor has many packages which support high-throughput sequence
analysis, including RNA sequencing (RNA-seq). The packages which we
will cover in this workflow include core packages maintained by the
Bioconductor core team for importing and processing raw sequencing
data and loading gene annotations. We will also cover
...
[@Himes2014RNASeq].  Glucocorticoids are used, for example, in asthma
patients to reduce inflammation of the airways. In the experiment,
...
**Scatterplot of transformed counts from two samples**. Using the log2
...
`r CRANpkg("ggplot2")` package (see Figures below).
...
Here the color indicates treatment.
...
are underpowered, and best excluded before multiple test correction.
...
as it will help to trace down what has happened in case an R script
