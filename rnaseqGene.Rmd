...
Michael Love [1], Simon Anders [2], Vladislav Kim [3], Wolfgang Huber [3]
...
The packages used in this workflow (which are loaded with the
*library* function) can be installed by following the
[Bioconductor package installation instructions](http://bioconductor.org/install/#install-bioconductor-packages).
Some R packages used here are not Bioconductor packages, but general-purpose
R packages hosted on CRAN (we will mention which ones are hosted on CRAN).
CRAN packages can also be installed using *biocLite*, or by using
*install.packages*.
...
*pheatmap* from the `r CRANpkg("pheatmap")` package hosted on CRAN.
...
`r CRANpkg("PoiClaClu")` package hosted on CRAN.
...
`r CRANpkg("ggplot2")` package [@Wickham2009Ggplot2] hosted on CRAN.
...
multiplicative factor of 2 raised to the 1.5 power, or approximately 2.82.
