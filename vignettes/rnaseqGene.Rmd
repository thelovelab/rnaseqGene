...
a faster, alternative pipeline to genome alignment followed by
counting reads in exons, and this is to import transcript-level
quantification, aggregating to the gene-level with *tximport* or
...
the `DESeqDataSet` function, which is demonstrated below.
...
```
salmon index -t transcripts.fa -i name_of_index
```

If the transcripts are downloaded from GENCODE, it is recommended to
use the following command (which simply helps to strip the extra
information from the transcript names):

```
salmon index --gencode -t transcripts.fa -i name_of_index
```
...
# "Untreated" => "untrt", "Dexamethasone" => "trt"
...
We can quickly check the millions of fragments that could be mapped by
*Salmon* to the genes (the second argument of *round* tells how many
decimal points to keep).
